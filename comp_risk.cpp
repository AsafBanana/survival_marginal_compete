...
void omega_expectation(double* __restrict__ rowSums, double* __restrict__ points, double* __restrict__ weights, double* __restrict__ hazared_at_event, double* __restrict__  beta_Z, int N, int N_Q, int N_E, int N_P, double* __restrict__ ret) {
	for (int i = 0; i < N; i++)
		for (int j = 0; j < N_E; j++) 
			ret[i*N_E + j] = 0;	
	double devisor[N];
...
// [[Rcpp::export]]
NumericVector omega_expectation_C(NumericMatrix rowSums, NumericMatrix P, NumericVector weights , NumericMatrix hazared_at_event , NumericMatrix beta_Z) {
	int N = rowSums.nrow();
	int N_E = rowSums.ncol();
	int N_P = P.nrow();
	int N_Q = hazared_at_event.ncol();
	...
	omega_expectation(rowSums_c,P_c,W_c,hazared_at_event_c,beta_Z_c,N,N_Q,N_E,N_P,ret);
...
void E_phase(double* __restrict__ rowSums, double* __restrict__ points, double* __restrict__ weights, double* __restrict__ hazared_at_event, double* __restrict__  beta_Z, int N, int N_Q, int N_E, int N_P, double* __restrict__ ret) {
	for (int i = 0; i < N_E; i++)
		for (int j = 0; j < N_E; j++) 
			ret[i*N_E + j] = 0;	
	#pragma omp parallel for
	for (int i = 0; i < N; i++) {
		...
		for (int j = 0; j < N_E; j++) {
			for (int m = 0; m < N_E; m++) {
				#pragma omp critical
				{
					ret[j*N_E + m] += PU_varcov[j][m]/devisor;
				}
			}
		}		
	}
...
// [[Rcpp::export]]
NumericVector E_phase_C(NumericMatrix rowSums, NumericMatrix P, NumericVector weights , NumericMatrix hazared_at_event , NumericMatrix beta_Z) {
	int N = rowSums.nrow();
	int N_E = rowSums.ncol();
	int N_P = P.nrow();
	int N_Q = hazared_at_event.ncol();
	...
	E_phase(rowSums_c,P_c,W_c,hazared_at_event_c,beta_Z_c,N,N_Q,N_E,N_P,ret);
